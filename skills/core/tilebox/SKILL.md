...
- Name the workflow and its project directory for the reusable capability, never for values or choices supplied by a job. Exclude locations, AOIs, dates, durations, events, scene counts, and any configurable cadence, dataset, algorithm, or output option. Apply the reuse test: if another valid job could change a word in the proposed name, omit that word. For example, a workflow whose inputs include AOI and cadence can be `rgb-timelapse`, not `vienna-seasonal-timelapse`; `seasonal-rgb-timelapse` is valid only when seasonal grouping is fixed workflow behavior rather than an input choice. Use `tilebox-workflow-releases` for the complete naming policy.
...
3. Use `tilebox-workflow-releases` to choose a reusable, non-parametric workflow name and run `tilebox workflow init` in the intended project directory if no project exists, honoring the initialization-order policy above.
4. Use `tilebox-workflow-authoring` for the task graph and processing code. Task-level parallelism is required whenever the outcome contains independent scenes, periods, AOIs, products, chunks, or tiles: infer the fanout from the request and make each unit a schedulable Tilebox task without requiring the user to prescribe the DAG. Use an explicit aggregation barrier when the output combines those units; never collapse them into a loop inside one task merely because the first run is small.
...
7. When the user asks for an actual map, video, detection set, or other processed result, use `tilebox-workflow-jobs` to resolve run-specific values into technical job inputs and submit a representative job. For a small job with a runner available, wait with `tilebox job wait` and return the verified final output; otherwise inspect or report its state. Compare the completed job's task summaries and execution statistics with the planned task graph; progress units or child spans inside one task do not demonstrate fanout. If the graphs do not match, fix the workflow before reporting completion. Deployment alone does not produce the requested artifact.
